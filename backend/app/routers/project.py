    ...
    try:
        ...
        async def render_task_async():
            try:
                ...
                # Initialize VideoEditor with temp and output directories
                video_editor = VideoEditor(temp_dir=TEMP_DIR, output_dir=OUTPUT_DIR)
                
                # Get the project's audio file path for voice-over
                voice_over_path = None
                if project_data and 'audio_file_path' in project_data:
                    voice_over_path = project_data['audio_file_path']
                    logger.info(f"Found voice-over path: {voice_over_path}")
                    # Check if the file exists
                    if voice_over_path and Path(voice_over_path).exists():
                        logger.info(f"Voice-over file exists: {voice_over_path}")
                    else:
                        logger.warning(f"Voice-over file not found: {voice_over_path}")
                else:
                    logger.warning(f"No audio_file_path found in project_data: {project_data}")
                
                # Run the video rendering process using VideoEditor
                output_path = await video_editor.render_video(
                    project_id=project_id,
