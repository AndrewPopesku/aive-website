...
class ProjectController(BaseController[ProjectRepository]):
    ...
    async def create_project_with_audio(self, session: AsyncSession, project_data: ProjectCreate) -> Project:
        """Create a new project with audio file."""
        # Validate audio file path exists
        if not project_data.audio_file_path:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Audio file path is required")

        # Check if project with same title already exists
        existing_project = await self.repository.get_by_title(session, project_data.title)
        if existing_project:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT, detail=f"Project with title '{project_data.title}' already exists"
            )
    ...
    async def get_project_with_details(self, session: AsyncSession, project_id: str) -> Dict[str, Any]:
        """Get project with all related data (sentences, footage choices, music)."""
        # Get the project
        project = await self.get_entity(session, project_id)
        ...
        music_recommendations = await self.music_repo.get_by_project_id(session, project_id)

        # Convert to response format
        project_dict = {
            "id": project.id,
            "project_id": project.id,  # Legacy compatibility
            "title": project.title,
            ...
            "videoUrl": project.video_url,  # Legacy compatibility
            "created_at": project.created_at.isoformat(),
            "updated_at": project.updated_at.isoformat(),
            "sentences": [self._sentence_to_dict(s) for s in sentences],
            "total_sentences": len(sentences),
            "footage_choices": [self._footage_choice_to_dict(fc) for fc in footage_choices],
            "music_recommendations": [self._music_recommendation_to_dict(mr) for mr in music_recommendations],
        }

        # Calculate total duration from sentences if not set
        if not project_dict["total_duration"] and sentences:
            project_dict["total_duration"] = sum(s.end_time - s.start_time for s in sentences)

        return project_dict

    async def add_sentences_to_project(
        self, session: AsyncSession, project_id: str, sentences_data: List[SentenceCreate]
    ) -> List[Dict[str, Any]]:
        ...
        # Create sentences
        sentences = await self.sentence_repo.create_multiple(session, project_id, sentences_dict)

        return [self._sentence_to_dict(s) for s in sentences]

    async def add_footage_choices(
        self, session: AsyncSession, project_id: str, footage_choices_data: List[FootageChoiceCreate]
    ) -> List[Dict[str, Any]]:
        ...
        choices = await self.footage_repo.create_multiple(session, project_id, choices_dict)

        return [self._footage_choice_to_dict(fc) for fc in choices]

    async def add_music_recommendations(
        self, session: AsyncSession, project_id: str, music_data: List[MusicRecommendationCreate]
    ) -> List[Dict[str, Any]]:
        ...
        recommendations = await self.music_repo.create_multiple(session, project_id, music_dict)
    ...
    async def update_sentence_footage(
        ...
        sentence = await self.sentence_repo.update_selected_footage(session, sentence_id, selected_footage)
        if not sentence:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Sentence with ID {sentence_id} not found"
            )

        return self._sentence_to_dict(sentence)

    def _sentence_to_dict(self, sentence) -> Dict[str, Any]:
    ...
    def _footage_choice_to_dict(self, footage_choice) -> Dict[str, Any]:
    ...
    def _music_recommendation_to_dict(self, music_rec) -> Dict[str, Any]:
