...
class BaseAPISet(Generic[ControllerType], ABC):
    """Base API set class for consistent route registration."""

    def __init__(self, prefix: str, controller: ControllerType, tags: list = None):
    ...
    def _setup_crud_routes(self, get_session_dependency):
        ...
        async def create_entity(data: dict, session: AsyncSession = Depends(get_session_dependency)):
            """Create a new entity."""
            return await self.controller.create_entity(session, data)

        @self.router.get("/{entity_id}")
        async def get_entity(entity_id: str, session: AsyncSession = Depends(get_session_dependency)):
            """Get an entity by ID."""
            return await self.controller.get_entity(session, entity_id)

        @self.router.get("/")
        async def get_entities(
            skip: int = 0, limit: int = 100, session: AsyncSession = Depends(get_session_dependency)
        ):
            """Get multiple entities with pagination."""
            return await self.controller.get_entities(session, skip, limit)

        @self.router.put("/{entity_id}")
        async def update_entity(entity_id: str, data: dict, session: AsyncSession = Depends(get_session_dependency)):
            """Update an entity by ID."""
            return await self.controller.update_entity(session, entity_id, data)

        @self.router.patch("/{entity_id}")
        async def patch_entity(entity_id: str, data: dict, session: AsyncSession = Depends(get_session_dependency)):
            """Partially update an entity by ID."""
            return await self.controller.update_entity(session, entity_id, data)

        @self.router.delete("/{entity_id}", status_code=status.HTTP_204_NO_CONTENT)
        async def delete_entity(entity_id: str, session: AsyncSession = Depends(get_session_dependency)):
...
class CRUDAPISet(BaseAPISet[ControllerType]):
    ...
    def __init__(self, prefix: str, controller: ControllerType, get_session_dependency, tags: list = None):
